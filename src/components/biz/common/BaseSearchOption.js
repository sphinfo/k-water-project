    ...
    return (
        <div className="base-search-wrap">
            <div className="content-row">
                <BaseSelectOption ref={selectRef} provider={namesRef.current} searchOn={(type) => {
                    dispatch({type: FLOOD_SEARCH_ON, searchOn: type});
                    dispatch({type: FLOOD_SELECT_BOX, selectBox: !type})
                }} changeItem={(item) => {
                    dispatch({type: FLOOD_SET_TEXT, text: item})
                }}/>
            </div>

            <div className="content-row">
                <div className="form-control group">
                    <BaseDatePicker date={dayjs('20100101')} maxDate={endDate} onchangeFromat={(date) => {
                        dispatch({type: FLOOD_START_DATE, date})
                    }}/>
                    <span>~</span>
                    <BaseDatePicker minDate={startDate} onchangeFromat={(date) => {
                        dispatch({type: FLOOD_END_DATE, date})
                    }}/>
                </div>
            </div>
        </div>
