...
import { G$holdMap } from '@gis/util';
...
const HoldMapWidget = () => {
    ...
    const { holdMap } = useSelector(state => state.main)
    ...
    return (
        <button className={`mapRightBt ${holdMap ? 'active' : ''}`} data-tooltip="지도 고정" onClick={()=>{ dispatch({type:HOLD_MAP, holdMap: !holdMap}) }}>
